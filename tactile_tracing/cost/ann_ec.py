...
class ContactArea:
    ...
    def _diff(self, target, base):
        ...
        diff[diff < 0.5] = (diff[diff < 0.5] - 0.5) * 0.8 + 0.5
...
def extract_surface_contact_real_time():
    ...
    while True:
        ...
        contact_area = ContactArea(base=src1, contour_threshold=10, real_time=True)
        contact_area(target=src2)  # __call__ method
        cv2.imshow('surface contact', src2)
        k = cv2.waitKey(1)
